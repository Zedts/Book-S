...
function SidebarBrand({ isCollapsed }: SidebarBrandProps) {
  return (
    <div className={cn("flex items-center gap-3", isCollapsed ? "justify-center flex-col px-0" : "px-2")}>
      ...
      {!isCollapsed && (
        <div>
          <h2 className="text-xl font-bold text-slate-800 leading-none">Admin</h2>
...
interface UserAccountProps {
  user: { id: string; role: string; fullName: string; avatarUrl?: string | null } | null;
  isCollapsed?: boolean;
  onLogout: () => void;
}
  ...
  return (
    <div className={cn(
      ...
      <div className={cn("flex items-center", isCollapsed ? "justify-center" : "gap-3 mb-3")}>
        <div className="w-10 h-10 rounded-full border-2 border-white shadow-sm overflow-hidden shrink-0 bg-slate-200 flex items-center justify-center">
            {user?.avatarUrl ? (
        ...
        {!isCollapsed && (
          <div className="flex-1 min-w-0 text-left">
            <p className="text-sm font-bold text-slate-800 truncate">{user?.fullName || "Administrator"}</p>
            <p className="text-[10px] font-medium text-slate-500 truncate">Super Admin</p>
          </div>
        )}
      </div>
...
export default function AdminSidebar({ isCollapsed, onToggle, isMobile }: AdminSidebarProps) {
  ...
  const navItems = [
    ...
    { href: "/admin/transactions", icon: Activity, label: "Pantau Transaksi" },
    { href: "/admin/orders", icon: ClipboardList, label: "List Pesanan" },
  ];
    ...
    return (
      <>
        ...
        >
          ...
          {/* Footer / Account */}
          <div className="mt-auto space-y-4 pt-6 border-t border-slate-200/50 shrink-0">
            <NavItem 
              href="/admin/settings" 
              icon={Settings} 
              label="Pengaturan" 
  ...
  return (
    ...
    >
      ...
      {/* Footer / Account */}
      <div className="mt-auto space-y-4 pt-6 border-t border-white/40 shrink-0">
        <NavItem 
          href="/admin/settings" 
          icon={Settings} 
          label="Pengaturan" 
